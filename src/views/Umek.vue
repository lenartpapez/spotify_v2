<template>
    <div class="component flex flex-wrap">
        <div class="w-1/4 p-4" v-for="track in tracks" :key="track.id">
            <span class="text-sm mr-2">{{ track.name }} <i @click="play(track.name, track.uri)" class="fas fa-play-circle text-red-500"></i></span>
      ...
      methods: {
        async getTracks () {
          await this.axios.get('https://api.spotify.com/v1/artists/5Hini2nQyoglzpdKe41cZt/top-tracks?country=US', {
            headers: {
              'Authorization': 'Bearer ' + this.localStorage.access_token
            }
          })
